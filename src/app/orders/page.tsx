...
export default async function OrdersPage({ searchParams }: Props) {
  ...
  if (!session?.user) return <div className="card p-6">Please sign in to view your orders.</div>;
    ...
    <div className="space-y-4">
      {success && (
        <div className="card p-4 bg-green-50 border-green-200">
          ...
          <div className="text-sm text-green-700">Your order {orderId ? <b>#{orderId}</b> : null} has been placed.</div>
        </div>
      )}
        ...
        orders.map((o: any) => (
          <div key={o._id} className="card p-4 space-y-3">
            ...
            <div className="text-xs text-gray-600">
              Placed on {new Date(o.createdAt).toLocaleString()} • Total ₹{(o.amount/100).toFixed(2)} • {o.payment?.method?.toUpperCase()} • ****{o.payment?.last4 || "0000"}
            </div>
